    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(test_loader)):
            ...
            if i % config.save_interval == 0:
                save_imgs(img, msk, out, key_points, gt_pre, i, os.path.join(config.work_dir, 'outputs', config.datasets), config.datasets, config.threshold, test_data_name=test_data_name)
